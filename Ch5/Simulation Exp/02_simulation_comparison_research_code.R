...
data.analysed = read.csv("glassdata_v1_prsim.csv", header = TRUE) ##ENTER THE FILE NAME
...
variables = 3 ##ENTER THE NUMBER OF THE COLUMN(S) THE VARIABLE(S) IS (ARE) LOCATED IN, E.G. 4 FOR UNIVARIATE OR 4,5 FOR MULTIVARIATE MODELS
...
n = length(unique(data.analysed$rep)) ##number of measurements per object
fp.Nor = 0
fp.KDE = 0
fn.Nor = 0
fn.KDE = 0
...
rownames(output.matrix.Nor) = as.character(unique(data.analysed$item))
colnames(output.matrix.Nor) = as.character(unique(data.analysed$item))

output.matrix.KDE = matrix(0,ncol = m.all.analysed, nrow = m.all.analysed)
rownames(output.matrix.KDE) = as.character(unique(data.analysed$item))
colnames(output.matrix.KDE) = as.character(unique(data.analysed$item))
...
{  
	...
	{	
		...
		{
			#AG EDIT
		  #Simulated data for recovered data, PCs for control data
		  #y.1.2 = data.analysed[which(data.analysed$item == i),] ##choosing an object described by n measurements which will be divided into two samples 'y.1' and 'y.2'
			#y.1 = data.frame(y.1.2[1:5,]) ##choosing y.1 ##ENTER THE NUMBER OF THE ROW(S) CREATING THE CONTROL SAMPLE, E.G. 1:6
			#y.2 = data.frame(y.1.2[6:10,]) ##choosing y.2 ##ENTER THE NUMBER OF THE ROW(S) CREATING THE RECOVERED SAMPLE, E.G. 7:12

			y.1 = data.frame(population[which(population$item == i),]) ##choosing 'y.1'
		...
		else ##When i is not equal to j, samples from different objects are compared (giving the false positive answer rates)
		{
		  ...
		  #data.analysed has the simulated data.
			y.1 = data.frame(population[which(population$item == i),]) ##choosing 'y.1'
			y.2 = data.frame(population[which(population$item == j),]) ##choosing 'y.2'
...
##Box-plots
boxplot(population[,variables], ylab = "data")

##Q-Q plots
for (k in 1:p) qqnorm(sapply(split(population[,variables[k]],population$item),mean), main = variables.names[k])
...
plot(bkde(sapply(split(population[,variables[k]],population$item),mean), kernel = "normal", bandwidth=h), type = "l", ylab = "probability density", xlab = "data", main = variables.names[k])
