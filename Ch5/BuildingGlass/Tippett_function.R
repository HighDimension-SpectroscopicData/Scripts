Tippett_plot = function(LR.H1.exp, LR.H2.exp)
{
  ...
  mycol <- rgb(255, 0, 0, max = 255, alpha = 178.5, names = "red70")
  ...
  polygon(c(tippett_col_fp$x.range, rev(tippett_col_fp$x.range)), c(tippett_col_fp$Tippett.2, rep(0,length(tippett_col_fp$Tippett.2))), col=mycol)
  polygon(c(tippett_col_fn$x.range, rev(tippett_col_fn$x.range)), c(rep(100,length(tippett_col_fn$Tippett.1)), rev(tippett_col_fn$Tippett.1)), col=mycol)
}
