histograms = function(LR.H1.exp, LR.H2.exp)
{
  ...
  xlim <- seq(round(min), round(max), by = 2.5)
  ...
  hist(log.LR.H1.exp, breaks=xbars-stepbins/2, col="blue", main="", xlab="", ylab="", axes = FALSE)
  axis(1, at = seq(round(min), max, by = 2.5), labels = seq(round(min), max, by = 2.5))
  axis(2)  
  ...
  title(ylab="Frequency", mgp=c(2.5,1,0),cex.lab=1)
  title(xlab="logLR", mgp=c(2.5,1,0),cex.lab=1)
  legend("topright", expression(paste(H[2]," true")), fill="darkorange") #topleft - only for v = 1
  abline(v = 0, lty = 2, lwd = 2)
