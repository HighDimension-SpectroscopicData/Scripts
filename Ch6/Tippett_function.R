Tippett_plot = function(LR.H1.exp, LR.H2.exp)
{
  ...
  log.LR.H1.exp = LR.H1.exp
  log.LR.H2.exp = LR.H2.exp
  ...
  plot(x.range, Tippett.2, type="s", xlab="", 
       ylab="", xlim=c(-3,3), ylim=c(0,100), lty=3, lwd = 2)
  ...
  title(ylab="Proportion of cases [%]", mgp=c(2.5,1,0),cex.lab=1)
  title(xlab=expression(paste(log[10],"LR greater than")), mgp=c(2.5,1,0),cex.lab=1)
  legend(x=-3, y=20, c(expression(paste("true-",H[2]," LR values")), expression(paste("true-",H[1]," LR values"))), lty=c(3,1), bty="n", lwd=c(2,2))
